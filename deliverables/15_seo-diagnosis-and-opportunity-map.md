...
| 3 | **修正 `lastmod` 日期** | 讓 sitemap 顯示「已更新」 | 🤖 已在分支完成 |
| 4 | **移除 `Disallow: /_next/`** | 讓 Googlebot 能渲染 | 🤖 已完成 |
| 5 | **為 8 個 404 舊網址加 301 重定向** | 回收流失的搜尋資產 | 🤖 可隨時做 |

### P1 — 內容：搶真正有搜尋量嘅字

| # | 行動 | 目標關鍵字 | 月搜尋量 |
|---|---|---|---|
| 6 | 寫一篇**香港網頁設計完整指南**（費用／流程／如何選公司／真實案例） | 網頁設計 | **22,200** |
...
3. **`www` → 非 `www` 301**：兩個 host 都回 200，建議統一（會改動正式 `.htaccess`）

...
const FORBIDDEN_PHRASES = [
  ...
  // 2026-09-21 正式站覆核發現：ComparisonTable.tsx 的「328%+」漏改（llms.txt 已改但組件漏）。
  // 已核准數字為「328%」，加「+」屬未經核准的上調 → 加入閘門，防止再發生。
  "328%+",
];
...
const NEGATION_MARKERS = [
  ...
  "沒有", "沒有人", "沒法", "無需", "不用",
  // 2026-09-21 新增：正式站覆核發現兩種句式被誤判為違規。
  //   ①「（部分供應商聲稱可以保證排名）無人能保證。」
  //   ②「Google 官方明確表示無人能保證排名」
  // 兩者都是標準的告誡句式。刻意用「無人能保證」而非裸「無人」——
  // 裸「無人」會令「市面上無人能像我們一樣保證排名」這類真違規漏檢。
  "無人能保證", "沒人能保證", "冇人能保證", "無代理能保證", "無人能作",
  // 告誡語：作者明顯站在讀者一方提醒
...
/**
 * 已核准的對外數字。
 * 2026-09-24：負責人逐項覆核全站成效宣稱後確認為真實（客戶身份不對外披露），
 * 以下數字加入白名單，避免每次 build 重複響同一批警報。新出現嘅數字仍會列 WARN 供覆核。
 */
const APPROVED_STATS = new Set([
  // 原有對外數字
  "500+", "328%", "98%", "4.9", "45+", "16", "100%", "24/7",
  // 2026-09-24 負責人確認（服務頁宣稱 + 案例頁成效 + 業界引用）
  "1.4%", "1.8x", "2.1x", "3.2x", "3,200+", "3,500+", "4.3x", "5.8x", "8x", "8.2%", "8.5x",
  "7%", "8%", "15%", "15+", "20%", "20+", "21.7%", "22%", "23%", "25%", "30%", "30+",
...
function selftest() {
  ...
  const MUST_PASS = [
    ...
    "避免相信「保證排名第一位」這類承諾。",
    // 真實案例：服務總覽表格（2026-09-21 build 攔到的兩處誤判）
    "誰可以保證結果（部分供應商聲稱可以保證排名）無人能保證。AI 平台輸出由平台決定。",
    "能否保證結果：不能。Google 官方明確表示無人能保證排名，不能。輸出由 AI 平台決定。",
  ];
  // 額外一組：驗證新加的否定標記不會造成假陰性。
  // 「無人能保證」是標記，但「無人能像我們一樣保證排名」是真違規 —— 必須攔到。
  const MUST_FLAG_EXTRA = [
    "市面上無人能像我們一樣保證排名，這就是我們的實力。",
  ];
  let bad = 0;
  console.log("閘門自我測試");
  console.log("─".repeat(64));
  for (const t of [...MUST_FLAG, ...MUST_FLAG_EXTRA]) {
  ...
  console.log(`\n✅ 自我測試通過（${MUST_FLAG.length + MUST_FLAG_EXTRA.length} 攔截 / ${MUST_PASS.length} 放行）。`);
  process.exit(0);
}
